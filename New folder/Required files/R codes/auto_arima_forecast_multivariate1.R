...
da<-read.csv("Book1.csv")
Category_Growth <- ts(da$growth , start = c(2015, 1),  frequency = 12)
#plot.ts(data)
library(forecast)

model1=auto.arima(Category_Growth)
model1
Category_Growth<-forecast(model1,13)
#plot(forecast(model2,12))


Category_Growth<-as.data.frame(Category_Growth)

write.csv(Category_Growth,file = "Book2.csv")
getwd()
